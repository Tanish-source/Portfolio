...
const Details = ({ type, time, place, info }) => {
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="sm:text-2xl text-xl font-cash font-semibold capitalize mb-2">{type}</h3>
        <span className="capitalize font-medium text-white/75">
          {time} | {place}
        </span>
        <p className="font-medium w-full text-white/75">{info}</p>
      </motion.div>
    </li>
  );
};
      ...
      <div className="my-40 mt-20 mb-0">
        ...
        <div ref={ref} className="md:w-[75%] mx-auto relative w-full">
          <motion.div
            ...
            className="absolute sm:left-9 left-6 top-0 sm:w-[4px] w-[3px] h-full bg-[#39689B] origin-top"
          />
